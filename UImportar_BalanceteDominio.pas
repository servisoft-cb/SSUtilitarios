unit UImportar_BalanceteDominio;
...
type
  TfrmImportar_BalanceteDominio = class(TForm)
...
var
  frmImportar_BalanceteDominio: TfrmImportar_BalanceteDominio;
...
procedure TfrmImportar_BalanceteDominio.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := Cafree;
end;

function TfrmImportar_BalanceteDominio.fnc_Montar_Campo: string;
var
  i: Integer;
  vTexto: string;
begin
...
procedure TfrmImportar_BalanceteDominio.FormShow(Sender: TObject);
begin
  fDMCadPlano_Contabil := TDMCadPlano_Contabil.Create(Self);
end;

function TfrmImportar_BalanceteDominio.fnc_Conta_Nivel(const Texto: string; Caracter: string): Integer;
var
  i, ivTot: Integer;
begin
  ivTot := 0;
...
function TfrmImportar_BalanceteDominio.fnc_Nivel_Superior(Conta: string; Nivel: Integer): Integer;
var
  i: Integer;
  Flag: Boolean;
begin
...
procedure TfrmImportar_BalanceteDominio.btnImportarClick(Sender: TObject);
begin
...
function TfrmImportar_BalanceteDominio.fnc_verifica_Arquivo(NomeArquivo,
...
procedure TfrmImportar_BalanceteDominio.prc_Le_XLS;
begin
...
function TfrmImportar_BalanceteDominio.Replace(Str, Ant, Novo: string): string;
var
  iPos: Integer;
begin
...
function TfrmImportar_BalanceteDominio.XlsToStringGrid2(AGrid: TStringGrid;
...
procedure TfrmImportar_BalanceteDominio.prc_Gravar_PlanoContabil;
var
  i, i2 : Integer;
  vTexto: String;
  vTexto2: String;
  ...
  vNome: String;
begin
  vcod_Reduzido := Monta_Numero(gGrid.Cells[0,Linha],0);
  ...
  fDMCadPlano_Contabil.cdsPlano_ContabilCODIGO.AsString   := gGrid.Cells[1,Linha];
  vNome := trim(gGrid.Cells[3,Linha]);
  if trim(gGrid.Cells[4,Linha]) <> '' then
    vNome := vNome + ' ' + trim(gGrid.Cells[4,Linha]);
  if trim(gGrid.Cells[5,Linha]) <> '' then
    vNome := vNome + ' ' + trim(gGrid.Cells[5,Linha]);
  if trim(gGrid.Cells[6,Linha]) <> '' then
    vNome := vNome + ' ' + trim(gGrid.Cells[6,Linha]);
  if trim(gGrid.Cells[7,Linha]) <> '' then
    vNome := vNome + ' ' + trim(gGrid.Cells[7,Linha]);
  if trim(gGrid.Cells[8,Linha]) <> '' then
    vNome := vNome + ' ' + trim(gGrid.Cells[8,Linha]);
  if trim(gGrid.Cells[9,Linha]) <> '' then
    vNome := vNome + ' ' + trim(gGrid.Cells[9,Linha]);
  fDMCadPlano_Contabil.cdsPlano_ContabilNOME.AsString := vNome;
  vTexto := gGrid.Cells[1,Linha];
  i2 := 0;
  for i := 1 to Length(vTexto) do
  begin
    if copy(vTexto,i,1) = '.' then
      i2 := i2 + 1;
  end;
  i2 := i2 + 1;
  fDMCadPlano_Contabil.cdsPlano_ContabilNIVEL.AsInteger := i2;
