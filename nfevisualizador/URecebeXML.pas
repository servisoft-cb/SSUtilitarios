...
begin
  ...
  begin
    ...
    begin
      if MessageDlg('*** Fornecedor com Saldo Crédito de ' + FormatFloat('###,###,##0.00',vVlrAux) + #13 +
    ...
    begin
      ...
      begin
        ...
        if StrToFloat(FormatFloat('0.00',fDMRecebeXML.cdsNotaFiscalVLR_SALDO_USADO.AsFloat)) > 0 then
          lbStatusContasPagar.Caption := 'Usado o Saldo do crédito!'
        else
...
begin
  ...
  begin
    ...
    begin
      if MessageDlg('Deseja buscar a próxima Sequencia da Referência?', mtConfirmation,[mbOk,mbNo],0) <> mrOk then
        CurrencyEdit2.AsInteger := fnc_Proxima_Ref;
    end;
  end;
  case ComboBox1.ItemIndex of
...
begin
  ...
  begin
    if MessageDlg('Deseja gerar as referências internas na sequência?', mtConfirmation,[mbOk,mbNo],0) = mrOk  then
      prc_Gerar_Ref;
  end;
end;
...
begin
  //12/07/2021
  vVlrAux := StrToFloat(FormatFloat('0.00',fnc_Saldo_Adto(fDMRecebeXML.vCodFornecedor_XML,'F')));
  if MessageDlg('*** Fornecedor com Saldo de Crédito de ' + FormatFloat('###,###,##0.00',vVlrAux) + #13 +
