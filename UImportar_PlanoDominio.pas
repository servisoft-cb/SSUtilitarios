...
type
  TfrmImportar_PlanoDominio = class(TForm)
...
var
  frmImportar_PlanoDominio: TfrmImportar_PlanoDominio;
...
procedure TfrmImportar_PlanoDominio.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := Cafree;
end;

function TfrmImportar_PlanoDominio.fnc_Montar_Campo: string;
var
  i: Integer;
  vTexto: string;
begin
...
procedure TfrmImportar_PlanoDominio.FormShow(Sender: TObject);
begin
  fDMCadPlano_Contabil := TDMCadPlano_Contabil.Create(Self);
end;

function TfrmImportar_PlanoDominio.fnc_Conta_Nivel(const Texto: string; Caracter: string): Integer;
var
  i, ivTot: Integer;
begin
  ivTot := 0;
...
function TfrmImportar_PlanoDominio.fnc_Nivel_Superior(Conta: string; Nivel: Integer): Integer;
var
  i: Integer;
  Flag: Boolean;
begin
...
procedure TfrmImportar_PlanoDominio.btnImportarClick(Sender: TObject);
begin
...
function TfrmImportar_PlanoDominio.fnc_verifica_Arquivo(NomeArquivo,
...
procedure TfrmImportar_PlanoDominio.prc_Le_XLS;
begin
...
function TfrmImportar_PlanoDominio.Replace(Str, Ant, Novo: string): string;
var
  iPos: Integer;
begin
...
function TfrmImportar_PlanoDominio.XlsToStringGrid2(AGrid: TStringGrid;
...
procedure TfrmImportar_PlanoDominio.prc_Gravar_PlanoContabil;
var
  i, i2 : Integer;
  vTexto: String;
  vTexto2: String;
  ...
  vNome: String;
begin
  vcod_Reduzido := gGrid.Cells[0,Linha];
  ...
  fDMCadPlano_Contabil.cdsPlano_ContabilCODIGO.AsString   := gGrid.Cells[7,Linha];
  i := 11;
  vNome := '';
  while trim(vNome) = '' do
  begin
    vNome := gGrid.Cells[i,Linha];
    i     := i + 1;
  end;
  fDMCadPlano_Contabil.cdsPlano_ContabilNOME.AsString := vNome;
  fDMCadPlano_Contabil.cdsPlano_ContabilNIVEL.AsInteger   := StrToIntDef(gGrid.Cells[20,Linha],0);
